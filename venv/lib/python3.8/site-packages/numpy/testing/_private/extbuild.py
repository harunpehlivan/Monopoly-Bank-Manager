...
def _c_compile(cfile, outputfilename, include_dirs=[], libraries=[],
    ...
    if sys.platform == 'darwin':
        ...
        for s in ('/sw/', '/opt/local/'):
            if f'{s}include' not in include_dirs and os.path.exists(
                f'{s}include'
            ):
                include_dirs.append(f'{s}include')
            if f'{s}lib' not in library_dirs and os.path.exists(f'{s}lib'):
                library_dirs.append(f'{s}lib')
