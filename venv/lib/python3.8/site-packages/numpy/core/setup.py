...
def check_svml_submodule(svmlpath):
    if not os.path.exists(f"{svmlpath}/README.md"):
...
def check_ieee_macros(config):
    ...
    def _add_decl(f):
        priv.append(fname2def(f"decl_{f}"))
        pub.append(f'NPY_{fname2def(f"decl_{f}")}')
...
def check_types(config_cmd, ext, build_dir):
    ...
    if not result:
        python = 'pypy' if '__pypy__' in sys.builtin_module_names else 'python'
        raise SystemError(
    ...
    for type in ('short', 'int', 'long'):
        if res := config_cmd.check_decl(
            f"SIZEOF_{sym2def(type)}", headers=["Python.h"]
        ):
            public_defines.append(
                (f'NPY_SIZEOF_{sym2def(type)}', f"SIZEOF_{sym2def(type)}")
            )

        else:
            res = config_cmd.check_type_size(type, expected=expected[type])
            if res >= 0:
                public_defines.append((f'NPY_SIZEOF_{sym2def(type)}', '%d' % res))
            else:
                raise SystemError(f"Checking sizeof ({type}) failed !")
    ...
    for type in ('float', 'double', 'long double'):
        ...
        if not already_declared and type != 'long double':
            private_defines.append((f'SIZEOF_{sym2def(type)}', '%d' % res))
        ...
        if res >= 0:
            public_defines.append((f'NPY_SIZEOF_COMPLEX_{sym2def(type)}', '%d' % res))
        else:
            raise SystemError(f"Checking sizeof ({complex_def}) failed !")
    ...
    if config_cmd.check_decl('PY_LONG_LONG', headers=['Python.h']):
        ...
        if res < 0:
            raise SystemError('Checking sizeof (PY_LONG_LONG) failed !')

        private_defines.append((f"SIZEOF_{sym2def('PY_LONG_LONG')}", '%d' % res))
        public_defines.append((f"NPY_SIZEOF_{sym2def('PY_LONG_LONG')}", '%d' % res))
        res = config_cmd.check_type_size('long long',
                expected=expected['long long'])
        if res >= 0:
            #private_defines.append(('SIZEOF_%s' % sym2def('long long'), '%d' % res))
            public_defines.append((f"NPY_SIZEOF_{sym2def('long long')}", '%d' % res))
        else:
            raise SystemError('Checking sizeof (long long) failed !')
...
def visibility_define(config):
    ...
    return hide if config.check_gcc_function_attribute(hide, 'hideme') else ''
...
def configuration(parent_package='',top_path=None):
    ...
    def generate_config_h(ext, build_dir):
        ...
        if newer(__file__, target):
            ...
            moredefs.append((f'HAVE_LDOUBLE_{rep}', 1))
    ...
    def generate_api_func(module_name):
        def generate_api(ext, build_dir):
            script = join(codegen_dir, f'{module_name}.py')
    ...
    def get_mathlib_info(*args):
        ...
        posix_mlib = ' '.join([f'-l{l}' for l in mlibs])
        msvc_mlib = ' '.join([f'{l}.lib' for l in mlibs])
    ...
    def gl_if_msvc(build_cmd):
        ...
        """
        return ['/GL-'] if build_cmd.compiler.compiler_type == 'msvc' else []
